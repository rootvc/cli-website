...
- The history nav MUST be at the TOP of the page, native to the week's theme, visible and signposted from page load. No "scroll to find it" / "secret door" / universal-strip approach. The Editor rejects incarnations that bury history nav at the bottom or use a generic cross-theme strip. See author-role.md's "Placement rule for the history nav" section — it applies to every cycle, every week, no exceptions. (Facts can stay as easter eggs; navigation cannot.)
