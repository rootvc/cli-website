...
Before spawning the Author, regenerate `archive/chain.json` so the shared wayback strip will see the new capture once it ships:

```bash
node scripts/cycle/rebuild-archive-index.js
```

The Author no longer wires its own prev/next nav — `/archive/wayback-nav.js` does it at runtime from `chain.json`. You still pass the catalog URL for optional in-world nods, but you no longer need to compute or pass previous-capture URLs.

Spawn the Author subagent (`scripts/cycle/prompts/author-role.md`) with these inputs:
- All configs (above)
- Performance log
- Topical brief (may say "no seed this week")
- **`archive_catalog_url`** — `/archive/` (always; optional in-world references can link here)
- **`random_capture_url`** — `/rand/` (optional; used if the Author writes an in-world "spin the dial" affordance)
- Retry counter (starts at 0; you accumulate Editor critiques here on retries)

(Historic: pre-wayback-strip cycles also passed `previous_drop_url` / `previous_drop_theme_name` / `previous_drop_date`. The Author prompt now ignores these; you may omit them. The chain.json regeneration above is still required so the strip surfaces the new capture once it ships.)
...
- The artifact MUST include `<script src="/archive/wayback-nav.js" defer></script>` in `<head>`. The shared strip is what surfaces the archive link the DC7 smoke test checks for; without it, the artifact ships without a history nav.
- The artifact MUST NOT compete with the wayback strip by building its own top-of-page prev/next/all-captures nav. An optional in-theme footer aside or `/rand/` easter egg is fine; a second top nav is not. See author-role.md's "Wayback nav strip" section.
- User-facing copy MUST NOT use the words "drop" or "issue" to refer to past or current incarnations. Translate to the current theme's parlance (editions / captures / flights / rooms / etc.). Internally the schema and code keep their existing names; this rule is about what visitors see. The Editor rejects incarnations whose visible copy uses the internal vocabulary. See author-role.md's "Naming rule for past artifacts" section.
