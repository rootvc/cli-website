...
### 5. Wayback nav strip (DC7 — mechanical pass)

The shared wayback strip now provides the canonical history entrance. Check:
- Does `index.html` include `<script src="/archive/wayback-nav.js" defer></script>` inside `<head>`? If missing, reject — the visitor lands without a history nav.
- Did the Author write their own top nav for prev/next/all-captures duties (a catalogue strip, departures board, version history sidebar, etc. occupying the top of the page)? If yes, reject — the wayback strip handles that role and any second top nav fights it visually.
- An optional in-theme footer aside ("past sheets stay in the binder," "spin the dial") is fine and doesn't need its own concept blurb. If `meta.history_view_concept` is present, evaluate it as a description of that optional nod, not a required entrance.
